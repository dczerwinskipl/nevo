...
- **React 19 & TypeScript:** Strict typing, function components, hooks, and modern concurrent primitives.
- **TanStack Router:** Type-safe, file-system routing generated via `@tanstack/router-generator`.
- **TanStack Query:** Asynchronous server state caching, background polling, and cache invalidation.
- **Tailwind CSS v4:** Direct-value semantic design tokens declared in `@theme static`, compiled via `@tailwindcss/vite`.
- **Radix UI:** Headless, accessible behavior primitives for dialogs, sheets, menus, tooltips, and portals.
- **Storybook 8 & Vitest:** Component stories co-located with implementation; browser-backed interaction and accessibility testing via `@storybook/addon-vitest` and `@vitest/browser-playwright`.
...
The dashboard frontend is organized into four hierarchical layers plus design system foundations:

$$\text{App} \longrightarrow \text{Routes} \longrightarrow \text{Features} \longrightarrow \text{Shared} \quad (\text{Foundations})$$
...
|---|---|---|---|
| **App** | `ui/app/`, `ui/App.tsx`, `ui/main.tsx` | Global bootstrap, root providers (`QueryClientProvider`, router context, theme shell, global error boundaries), root layout frame. | `routes`, `features`, `shared`, `foundations` |
| **Routes** | `ui/routes/` | File-system routing entry points. Thin orchestrators extracting URL params and route search state, passing them to feature views. | `features`, `shared`, `foundations` |
| **Features** | `ui/features/<domain>/` | Vertical slices owning domain workflows, views, container components, local UI state, queries, mutations, projections, and co-located stories. | `shared`, `foundations`, own feature internals |
| **Shared** | `ui/shared/`, `ui/components/ui/` | Reusable, domain-independent UI primitives (`Button`, `Dialog`, `StatusLabel`), markdown renderers, token contracts (`status-tone.ts`), and utility helpers (`cn`). | `foundations`, external libraries (Radix, Lucide) |
| **Foundations** | `ui/foundations/`, `ui/index.css` | Semantic color tokens, typography scales, global CSS resets, and design system catalog stories. | External styling dependencies |

### Dependency Rules

- **Downward-Only Flow:** Upper layers may import from lower layers. Lower layers must **never** import from upper layers.
...
Clear import paths maintain codebase maintainability and avoid accidental cyclic dependencies.

### 1. Relative vs. Root-Relative Imports

- **Within the same feature or folder:** Always use **relative imports**:
  ```typescript
  ...
  ```
- **Cross-boundary imports:** Always use root-relative imports via the `@/` path alias:
  ```typescript
...
- **Rule:** Do not create catch-all `index.ts` barrel files that re-export an entire folder of components.
- **Rationale:** Barrel files obscure dependency trees, slow down TypeScript compilation and bundler tree-shaking, and easily introduce circular dependency cycles. Import symbols directly from their defining module.

### 3. No Deep Feature Imports
...
- **Migration Plan:** A dedicated future architectural change will mechanically move all files from `components/ui/` to `shared/ui/`, update import statements, and delete `components/ui/`.
...
| **Features** | `Features/<Domain>/<Component>` | `Features/Specifications/Status Board`<br>`Features/Specifications/Specification List`<br>`Features/Pull Requests/Summary Card`<br>`Features/Operations/Step Row`<br>`Features/Agent Sessions/Chat Surface`<br>`Features/Agent Sessions/Delete Session Scenario` |
...
## 10. Directory Structure (Pseudo-Tree)

The complete dashboard directory layout:

```text
tools/dashboard/
├── .storybook/
│   ├── test-utils/                      # Test infrastructure & helpers
...
├── ui/
│   ├── app/                             # Top-level composition & providers
│   ├── routes/                          # File-system route endpoints
│   ├── features/                        # Domain-driven vertical slices
│   │   ├── agent-sessions/              # AI sessions & chat surface
│   │   ├── specifications/              # Specification console & board
│   │   ├── pull-requests/               # Pull request & diff views
│   │   └── operations/                  # Background operation progress
│   ├── shared/                          # Domain-agnostic shared layer
│   │   ├── ui/                          # Official target for shared primitives
│   │   │   ├── loading-screen.tsx
│   │   │   ├── loading-screen.stories.tsx
│   │   │   └── status-label.tsx
│   │   ├── markdown/                    # Markdown content renderer
│   │   └── status-tone.ts               # Semantic status tone contract
│   ├── components/
│   │   └── ui/                          # Legacy primitive directory (shadcn)
...
│   ├── foundations/                     # Design system foundation stories
│   │   ├── colors.stories.tsx
...
└── vitest.config.ts                     # Vitest unit & Storybook test runner
```
...
- **Consolidation of `components/ui` to `shared/ui`:** Mass relocation of the Radix primitives from `components/ui/` to `shared/ui/` is explicitly deferred to a dedicated mechanical change.
