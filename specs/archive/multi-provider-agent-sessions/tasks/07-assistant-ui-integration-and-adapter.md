...
- Implement `NevoAssistantRuntime` in `tools/dashboard/src/lib/nevo-assistant-runtime.ts` fetching initial message thread and pending interaction from `GET /api/agent-sessions/:provider/:providerSessionId`, consuming live SSE stream events via `/api/agent-sessions/:provider/:providerSessionId/events` applying only events newer than `lastEventSeq` cursor, and delegating turn execution/cancellation to NEvo API.
