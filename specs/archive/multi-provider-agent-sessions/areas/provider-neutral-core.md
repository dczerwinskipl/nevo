...
- **Guaranteed forced cancellation:** `cancelTurn` requests adapter-level cancellation, then waits a bounded grace period for the operation to actually stop before escalating to a forceful kill (`SIGINT` -> bounded grace period -> explicit `SIGKILL` if still alive, followed by a second bounded wait for terminal exit) — the turn always reaches a terminal state on an explicit user cancel request or watchdog timeout, never leaving an orphaned OS process the user has no way to stop.
