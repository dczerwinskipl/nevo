...
Harden `ClaudeAgentProvider` by supplying curated baseline model metadata and operator configuration, supporting turn-level model overrides with open permissive passthrough, adopting OS-aware process tree spawn options and termination, and mapping CLI errors into the normalized failure taxonomy.
...
- Update child process spawn to adopt shared process tree spawn options (`detached: process.platform !== 'win32'`) from `process-termination.mjs`, and integrate `terminateChildProcess` to guarantee complete process tree cleanup across Windows and POSIX during cancellation or timeout.
...
1. Claude adapter exposes curated and configured models as `AgentModelDescriptor[]`. `automated: node --test tools/dashboard/tests/claude-provider.test.mjs`
2. Turn execution with specified model passes `--model` flag to `claude`; omitting it leaves provider defaults. `automated: node --test tools/dashboard/tests/claude-provider.test.mjs`
3. Permissive passthrough allows unlisted model strings to pass to `--model` without validation failure. `automated: node --test tools/dashboard/tests/claude-provider.test.mjs`
4. Turn cancellation invokes OS-aware process tree termination with process-group isolation, ensuring nested tools are cleaned up. `automated: node --test tools/dashboard/tests/claude-provider.test.mjs`
5. CLI non-zero exit codes and error messages map to normalized failure codes and neutral recovery hints. `automated: node --test tools/dashboard/tests/claude-provider.test.mjs`
6. Fastify MCP interactions correlate cleanly with neutral interaction schemas. `automated: node --test tools/dashboard/tests/claude-provider.test.mjs`
