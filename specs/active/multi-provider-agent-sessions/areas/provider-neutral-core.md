...
- **Guaranteed forced cancellation:** `cancelTurn` requests adapter-level cancellation, then waits a bounded grace period for the operation to actually stop before escalating to a forceful kill (unsignaled `child.kill()`, the only reliable forced-termination path on Windows) — the turn always reaches a terminal state on an explicit user cancel request, never leaving an orphaned OS process the user has no way to stop.
