...
5. **Short-lived turn process & interaction deferral**: Turns execute via short-lived CLI processes. AskUserQuestion uses the verified PreToolUse/defer roundtrip. Native Claude permission transport is selected by the targeted Task 03 discovery.
...
- **C5.** Provider-owned lifecycle & short-lived processes: Providers own their session lifecycles. Turn execution spawns or resumes short-lived CLI processes. AskUserQuestion uses the verified PreToolUse/defer roundtrip. Native Claude permission transport is selected by the targeted Task 03 discovery.
