...
## D18: Persisted workflow-step progress is new, Git-tracked task state, distinct from task lifecycle `status` — **owner confirmation requested**
...
- **Decision (recommended, pending explicit confirmation):** Option 3 — Git-tracked `workflow_progress` on the task entry. Reasoning: unlike the finish-operation record (D14), which exists solely to survive a crash *inside one finalize call* (a window measured in seconds, on one machine, with no audit value once resolved), a task's current workflow step is long-lived, domain-relevant, cross-session state — a task can realistically sit at "review" for hours or days, be picked up by a different agent session or machine, and needs to be visible in the same place `status` already is (PR diffs, `change.yaml` itself) rather than trapped in a git-ignored local directory that a fresh clone or CI runner would never see. The `update-task` finalize stage (Task 06) already writes to `change.yaml` and commits that write together with the implementation (C14) — extending it to also write `workflow_progress` is a small, consistent generalization of a mechanism this specification already built and verified, not a new one.
  - **This is flagged, not silently decided, because it is a persistence-ownership choice** (`AGENTS.md`'s owner-approval-required list). Tasks 08-12 (added by this correction pass) are written assuming Option 3; if the owner prefers Option 2 instead, only Task 08's design needs to change before implementation starts — nothing else in Tasks 09-12 depends on which option was chosen, since they all consume `workflow_progress` through the same `resolveCurrentStepName`-equivalent function regardless of where it's stored.
