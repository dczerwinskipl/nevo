...
- **C27.** Current-step resolution checks, in this fixed order, whether the task's lifecycle `status` already equals one of the workflow definition's valid terminal transition targets before consulting `workflow_progress` or falling back to `entryStep` — a completed task must never be indistinguishable from one that never started the workflow. `workflow_progress` is never cleared or nulled at terminal completion; it remains as historical evidence, inert to further resolution once `status` is terminal (D28).
...
### 14. Remaining Scope: True Multi-Step Workflow Orchestration (D18-D31)
...
**Genuinely new:**
- **Persisted multi-step progress with explicit terminal precedence (D18 approved/D19/
  D28, C21/C22/C27):** a `workflow_progress` field on the task's `change.yaml` entry,
  distinct from task lifecycle `status`, tracking which declared step a task is
  currently on; a transition's `to` resolves against the definition's own step names
  first, falling back to a real, validated terminal lifecycle status only when no step
  matches; current-step resolution checks `status` against the definition's terminal
  targets *before* consulting `workflow_progress`/`entryStep`, so a completed task can
  never look like a fresh one, and `workflow_progress` is never cleared — it remains as
  history.
...
- **Explicit transition/entry-step model with real-status validation (D27, D19 refined,
  C22):** exactly one `transitions` entry per step (no more silently-ignored entries
  after index 0), a terminal `to` value that must be a genuine member of
  `TASK_STATUSES` (never a typo), and an optional, explicit `entryStep` field (falling
  back to the first declared step when omitted).
...
- **Remaining scope (Tasks 08-12, added 2026-09-08, revised twice 2026-09-08 — see D18-D31):**
  ...
  - A step transition's `to` resolves against the definition's own declared step names before falling back to a terminal lifecycle-status write that must be a genuine member of the repository's `TASK_STATUSES` — a typo can never reach `task.status`; every step declares exactly one transition, and an optional explicit `entryStep` names the workflow's entry point; today's single-step `standard.yaml` behaves identically before and after this generalization (C22).
