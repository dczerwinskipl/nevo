...
closing the one thing Tasks 01-12 individually generalize/enable but never collectively
demonstrate together (`areas/multi-step-workflow-orchestration.md` §7). This is the
acceptance test for the whole multi-step correction (D18-D37), the same role Task 07
played for the single-step foundation.

## Implementation constraints

- **This task adds no new engine code** — `allowed_paths` is test files and docs only.
  If proving the scenario reveals a real defect in Tasks 08-12's implementation, fix it
...
1. `workflow step start` resolves step A for a fresh task on a ≥3-step fixture
   definition, persisting `current_step: A, state: active` (D37 case A). `automated: node --test tools/tests/workflow-multi-step-e2e.test.mjs`
2. **Active/completed checkpoint (D37):** finishing A (`workflow step finish`) persists
   `workflow_progress.state = completed` on step A — `current_step` stays `A` — in the
   same commit as the implementation, appending a `history` entry with
   `transitioned_to: B`; a `workflow step start` call *before* this finish (case B,
   resume) and a repeated `step finish` call *after* it (already-completed, non-actionable,
   no finalize re-run) are both exercised. Only the *next* `workflow step start` call
   actually activates B (`current_step: B, state: active`, no new `history` entry) —
   resolving B, not A. `automated: node --test tools/tests/workflow-multi-step-e2e.test.mjs`
...
6. **Terminal precedence, driven via CLI (D28, corrected by D37):** finishing C reaches
   the terminal case — `task.status` is written, `workflow_progress.state = completed`
   and `current_step` is left populated (naming the last real step, never
   cleared/nulled), all atomically, and the next `workflow step start` reports the
   workflow already complete — never re-resolving `entryStep` as if the task were
   starting fresh, and never consulting `task.status` to make that determination (D37 —
   position resolution is a pure function of `workflow_progress`/`definition`).
