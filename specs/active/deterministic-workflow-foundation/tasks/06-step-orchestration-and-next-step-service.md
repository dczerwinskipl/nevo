...
Implement declarative step orchestration in `tools/specs/workflow/step-runner.mjs` and build the deterministic "What next?" query service in `tools/specs/workflow/next-step.mjs` that supplies agents with complete, factual next-step guidance directly from runtime state.

## Implementation constraints

- Support composing declarative steps with entry gates, actions, exit gates, and transitions.
- The `getNextStep` service must inspect change state, active step, gate results, action checks, and human verification status.
