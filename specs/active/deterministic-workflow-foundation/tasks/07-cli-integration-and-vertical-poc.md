...
semantic_references:
  decisions: [D1, D6, D8]
  constraints: [C1, C2, C5, C6, C7, C8, C9, C10]
---
...
Integrate the deterministic workflow engine and commands into `tools/specs.mjs`, execute the vertical proof-of-concept (`CommandGate` test + `HumanVerificationGate` + `finalize: [verify-task-output, commit-and-push]`), and verify full coexistence and zero regressions with the legacy workflow.
...
- Expose deterministic workflow commands (e.g. `node tools/specs.mjs workflow next-step <change>`, `node tools/specs.mjs workflow execute-step <change> <task> <step>`, and `--check` on step actions).
- Execute the vertical PoC multi-action implementation/finalize flow end-to-end against fixture repositories.
...
2. Multi-step finalize vertical PoC executes end-to-end under deterministic mode: non-mutating check aggregates contracts, gate inspection does not run tests, fail-closed rejects missing commit messages and missing include selections, human verification blocks until confirmed, and valid execution completes the finalize step. `automated: node --test tools/tests/workflow-e2e.test.mjs`
