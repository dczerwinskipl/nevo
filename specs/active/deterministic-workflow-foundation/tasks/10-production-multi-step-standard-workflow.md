...
semantic_references:
  decisions: [D7, D18, D19, D20]
  constraints: [C9, C21, C22]
...
while behaving identically to before.

**Exact step names, count, and gate composition are this task's own implementation
decision** — `areas/multi-step-workflow-orchestration.md` §5 gives a non-binding,
illustrative shape only. Before implementing, record the real decomposition as its own
entry in `owner-decisions.md` (what steps, what gates each one owns, why), following
this repository's decision-policy — do not silently pick a shape and implement it
without that record.
...
- The final step's transition target must be a task lifecycle status value (the
  terminal case, D19) — never accidentally another step's name.
...
1. `owner-decisions.md` records the real step decomposition for Standard (names, gate
   ownership per step, rationale) before the definition is implemented. `automated: node tools/docs.mjs validate`
2. `.nevo-ai/workflows/standard.yaml` declares at least three steps, each with its own
   gates, and the final step's transition targets a genuine terminal task-status value.
...
3. Loading the new definition succeeds under Task 09's fail-closed validation (every
   action id and gate type referenced is registered). `automated: node --test tools/tests/workflow-e2e.test.mjs`
4. `docs/development/workflow-engine.md` reflects the real multi-step shape (no stale
   single-step example left as if it were current). `automated: node tools/docs.mjs check`
5. Every existing Task 06/07/08/09 test that constructs its own inline fixture
