...
forbidden_paths:
  ...
  - tools/dashboard/src/**
semantic_references:
  decisions: [D2, D9, D10, D11]
  constraints: [C1]
  dependency_contracts: [operation-progress-contract-and-transport, cli-step-instrumentation-gate-and-verification]
---

# Task: Approve post-action sync and Git integration

## Goal

Expand task approval (`handleApprove` in `tools/specs.mjs`) into a full, genuine multi-step
CLI operation:
...
This provides a safe, reproducible, non-destructive multi-step operation for both CLI users
and the dashboard frontend (Task 07).
...
- Git integration defaults to `true`. CLI option `--no-git` disables commit and push.
