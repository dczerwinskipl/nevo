...
- `workflow step start`/`workflow step finish`/`workflow verify-human` invoked without a
  task id on a deterministic spec must fail with a clear "task id required" error naming the
...
- `workflow step finish <change>` and `workflow verify-human <change>` (task id omitted)
  fail identically. `automated: node --test tools/tests/deterministic-cli-default-task-resolution.test.mjs`
