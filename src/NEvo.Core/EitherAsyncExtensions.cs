...
public static class EitherAsyncExtensions
{
    ...
    /// <c>Right(None)</c> becomes <c>Left</c> via the supplied <paramref name="onNone"/>
    /// factory.
    /// </summary>
    public static EitherAsync<TLeft, TRight> RequireSome<TLeft, TRight>(
        this EitherAsync<TLeft, Option<TRight>> source,
        Func<TLeft> onNone
    ) => source.Bind(option => option.Match(
        Some: value => RightAsync<TLeft, TRight>(value),
        None: () => LeftAsync<TLeft, TRight>(onNone())
    ));
}
