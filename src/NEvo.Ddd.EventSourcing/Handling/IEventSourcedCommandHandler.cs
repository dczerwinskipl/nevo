...
/// dependency. An explicit Event Sourced handler may return events directly or
/// delegate domain decision logic to <see cref="Deciding.IAggregateMethodDecider"/>.
/// The framework still owns load/replay/version/append/publish regardless of which
/// path a handler takes. Manages exactly one Event Sourced write target per command —
/// this type's own shape offers no way to write a second, independently-versioned
/// stream in the same invocation.
/// </summary>
