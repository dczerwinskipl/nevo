...
describe('Deterministic dependency satisfaction (Task 11, D9)', () => {
  ...
  describe('D40 Release epochs and explicit invalidation', () => {
    const epochWorkflow = {
      id: 'epoch-wf',
      entryStep: 'implement',
      steps: {
        implement: {
          executor: 'agent',
          transitions: [
            { to: 'review', releasesDependencies: true },
          ],
        },
        review: {
          executor: 'agent',
          transitions: [
            { value: 'pass', to: 'signoff' },
            { value: 'fail', to: 'implement', invalidatesDependencyRelease: true },
          ],
        },
        signoff: {
          executor: 'human',
          transitions: [
            { value: 'approve', to: 'verified', outcome: 'success' },
            { value: 'changes-requested', to: 'implement', invalidatesDependencyRelease: true },
          ],
        },
      },
    };
    ...
    const epochChange = {
      id: 'epoch-change',
      workflow: { mode: 'deterministic', definition: 'epoch-wf' },
      workflowDefinition: epochWorkflow,
    };

    test('Dependency released at implement -> review remains satisfied across further non-invalidating transition', () => {
      const depTask = {
        id: 'dep-task',
        workflow_progress: {
          current_step: 'signoff',
      ...
      const result = evaluateDependencySatisfaction(depTask, epochChange, epochWorkflow);
      assert.equal(result.satisfied, true);
      assert.deepEqual(result.releaseEpoch, { step: 'implement', attempt: 1 });

      const check = checkTaskDependencies({ id: 'downstream', depends_on: ['dep-task'] }, epochChange, [depTask], epochWorkflow);
      assert.equal(check.satisfied, true);
    });

    test('Declared invalidation transition revokes dependency release', () => {
      const depTask = {
        id: 'dep-task',
        workflow_progress: {
          current_step: 'implement',
          ...
          history: [
            { step: 'implement', attempt: 1, transitioned_to: 'review' },
            { step: 'review', attempt: 1, result: 'fail', transitioned_to: 'implement' },
          ],
        },
      };
      ...
      assert.ok(result.reason.includes('active on step \'implement\''));
    ...
    test('Human verification request changes (invalidatesDependencyRelease) revokes release', () => {
      const depTask = {
        id: 'dep-task',
        workflow_progress: {
          current_step: 'implement',
          ...
          history: [
            { step: 'implement', attempt: 1, transitioned_to: 'review' },
            { step: 'review', attempt: 1, result: 'pass', transitioned_to: 'signoff' },
            { step: 'signoff', attempt: 1, result: 'changes-requested', transitioned_to: 'implement' },
          ],
        },
      };
    ...
    test('Re-release after invalidation creates a new valid release epoch', () => {
      const depTask = {
        id: 'dep-task',
        workflow_progress: {
          current_step: 'review',
          ...
          history: [
            { step: 'implement', attempt: 1, transitioned_to: 'review' },
            { step: 'review', attempt: 1, result: 'fail', transitioned_to: 'implement' },
            { step: 'implement', attempt: 2, transitioned_to: 'review' },
          ],
        },
      };
      ...
      const result = evaluateDependencySatisfaction(depTask, epochChange, epochWorkflow);
      assert.equal(result.satisfied, true);
      assert.deepEqual(result.releaseEpoch, { step: 'implement', attempt: 2 });
    });
  });

  describe('D58 Dependency consumption & sequence-based authoritative matching', async () => {
    const { recordDependencyConsumption, findConsumersOfEpoch } = await import('../specs/workflow/dependency-consumption.mjs');
    ...
    test('Setup temp repo', () => {
      tmpDir = mkdtempSync(path.join(tmpdir(), 'nevo-dep-consumption-test-'));
    });

    test('A task depending on two release-based upstream tasks records both in one atomic write with one consumptionSequence', () => {
      recordDependencyConsumption({
        repoRoot: tmpDir,
        change: 'test-change',
        consumingTaskId: 'downstream-task',
        consumingStep: 'implement',
        consumingAttempt: 1,
        consumptionSequence: 1,
        dependencies: [
          { taskId: 'upstream-1', releaseEpoch: { step: 'implement', attempt: 1 } },
          { taskId: 'upstream-2', releaseEpoch: { step: 'implement', attempt: 1 } },
        ],
      });

      // Invalidating upstream-1 finds the consumer
      const consumers1 = findConsumersOfEpoch({
        repoRoot: tmpDir,
        change: 'test-change',
        dependencyTaskId: 'upstream-1',
        releaseEpoch: { step: 'implement', attempt: 1 },
      });
      assert.equal(consumers1.length, 1);
      assert.equal(consumers1[0].taskId, 'downstream-task');

      // Invalidating upstream-2 also finds the consumer
      const consumers2 = findConsumersOfEpoch({
        repoRoot: tmpDir,
        change: 'test-change',
        dependencyTaskId: 'upstream-2',
        releaseEpoch: { step: 'implement', attempt: 1 },
      });
      assert.equal(consumers2.length, 1);
      assert.equal(consumers2[0].taskId, 'downstream-task');
    });

    test('Task records dependency consumption on a later attempt (rework) and sequence-based matching finds only the authoritative epoch', () => {
      // Downstream task attempt 2 consumes fresh epoch { step: 'implement', attempt: 2 } at sequence 2
      recordDependencyConsumption({
        repoRoot: tmpDir,
        change: 'test-change',
        consumingTaskId: 'downstream-task',
        consumingStep: 'implement',
        consumingAttempt: 2,
        consumptionSequence: 2,
        dependencies: [
          { taskId: 'upstream-1', releaseEpoch: { step: 'implement', attempt: 2 } },
        ],
      });

      // Invalidating the higher-consumptionSequence epoch (attempt 2) finds consumer
      const consumersEpoch2 = findConsumersOfEpoch({
        repoRoot: tmpDir,
        change: 'test-change',
        dependencyTaskId: 'upstream-1',
        releaseEpoch: { step: 'implement', attempt: 2 },
      });
      assert.equal(consumersEpoch2.length, 1);
      assert.equal(consumersEpoch2[0].taskId, 'downstream-task');
      assert.equal(consumersEpoch2[0].consumptionSequence, 2);

      // Invalidating earlier superseded epoch (attempt 1) does NOT find consumer
      const consumersEpoch1 = findConsumersOfEpoch({
        repoRoot: tmpDir,
        change: 'test-change',
        dependencyTaskId: 'upstream-1',
        releaseEpoch: { step: 'implement', attempt: 1 },
      });
      assert.equal(consumersEpoch1.length, 0);
    });

    test('Two distinct consuming steps ordered correctly by consumptionSequence, not attempt number', () => {
      // Step A attempt 2 had consumptionSequence 5, Step B attempt 1 had consumptionSequence 10
      recordDependencyConsumption({
        repoRoot: tmpDir,
        change: 'test-change',
        consumingTaskId: 'multi-step-consumer',
        consumingStep: 'step-a',
        consumingAttempt: 2,
        consumptionSequence: 5,
        dependencies: [
          { taskId: 'dep-x', releaseEpoch: { step: 'implement', attempt: 1 } },
        ],
      });
      ...
      recordDependencyConsumption({
        ...
        change: 'test-change',
        consumingTaskId: 'multi-step-consumer',
        consumingStep: 'step-b',
        consumingAttempt: 1,
        consumptionSequence: 10,
        dependencies: [
          { taskId: 'dep-x', releaseEpoch: { step: 'implement', attempt: 2 } },
        ],
      });

      // Authoritative is sequence 10 (epoch 2) even though attempt was 1
      const consumersEpoch2 = findConsumersOfEpoch({
        repoRoot: tmpDir,
        change: 'test-change',
        dependencyTaskId: 'dep-x',
        releaseEpoch: { step: 'implement', attempt: 2 },
      });
      assert.equal(consumersEpoch2.length, 1);
      assert.equal(consumersEpoch2[0].consumptionSequence, 10);
