 ...
 *
 * `paths` (see paths.mjs's `resolveSpecsPaths`, resolved once by
 * specs/routes.mjs) carries the exact same `activeDir`/`archiveDir` the
 * rest of the Specs capability already resolved — this is the one place
 * that maps them to the watcher's explicit `{ path, prefix }` roots, so a
 * custom Specs directory configuration (e.g. a test fixture or a worktree)
 * is watched, not the repo's own real `specs/active`/`specs/archive`
 * regardless of configuration. No default-resolution happens here or in
 * the (fully generic) watcher implementation — only at the outer Specs
 * route boundary. `watcher` is this plugin's own local override option (a
 * feature-level test seam); real usage never passes one, so a real
