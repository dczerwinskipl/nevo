...
import {
  ...
  mapGitHubFileManifest,
  mapGitHubPullRequest,
} from '../server/pull-requests/github.mjs';
  ...
  return [
    {
      ...
      raw_url: 'https://raw.example/new',
      blob_url: 'https://blob.example/new',
    },
    {
...
test('pull-requests service isolates success, unsupported providers, and sanitized errors', async () => {
  ...
  writeFileSync(
    join(activeDir, 'mixed-pr', 'change.yaml'),
    [
      'id: mixed-pr',
      'title: Mixed PR',
      'pull_requests:',
...
test('loads exact active and archived changes and skips the provider for empty references', async () => {
  ...
  writeFileSync(
    join(activeDir, 'with-pr', 'change.yaml'),
...
test('loads a PR sub-resource (files/file-diffs/full-diff) by number, resolved from change.yaml', async () => {
  ...
  try {
    ...
    const diffs = await service.loadFileDiffs({
      source: 'active',
      slug: 'with-pr',
      number: '42',
      paths: ['a.js'],
