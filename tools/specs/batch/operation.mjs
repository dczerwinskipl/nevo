...
export function startBatch(changeSlug, mode, options = {}) {
  ...
  return {
    ...
    checkpointTask,
    orderedTasks: selection.orderedTasks,
    firstTask: selection.orderedTasks[0],
  };
}

export function getBatchStatus(changeSlug) {
  const change = requireChange(changeSlug);
  const intent = loadBatchIntent(change);
  if (!intent) {
    return { change: changeSlug, active: false };
  }
...
export function reviewBatch(changeSlug, options = {}) {
  ...
  const verdict = computeBatchReviewVerdict({
    ownerDecisionFindings: openBlocking.length,
    otherFindings: integrationFindings.length,
  });
  ...
  return {
    ...
    batchId,
    reportFile,
    relativeReportPath: `specs/active/${changeSlug}/reviews/batch-${batchId}.md`,
    verdict,
    openBlocking,
    integrationFindings,
  };
