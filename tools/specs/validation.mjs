...
import {
  listChanges, ACTIVE_DIR, ARCHIVE_DIR, ROOT,
} from './store.mjs';
...
export function validateWorkflowProgress(change, task, errors, label, { repoRoot = ROOT } = {}) {
  ...
  if (resolvedMode.mode !== 'deterministic') {
    errors.push(
      `${label}: workflow_progress is present but this change resolves to workflow mode ` +
      `'${resolvedMode.mode}' — workflow_progress is only valid on a 'workflow.mode: deterministic' change`
    );
    return;
  }

  if (!isPlainObject(task.workflow_progress)) {
    errors.push(`${label}: workflow_progress must be an object`);
    return;
  }
  ...
  try {
    ...
    errors.push(
      `${label}: could not resolve workflow definition '${resolvedMode.definition}' to validate ` +
      `workflow_progress.current_step: ${err.message}`
    );
    return;
  }
  ...
  if (Array.isArray(history)) {
    ...
    history.forEach((h, idx) => {
      ...
      if (h.attempt === undefined || h.attempt === null || !Number.isInteger(h.attempt) || h.attempt < 1) {
    ...
    if (state === 'completed') {
      if (history.length === 0) {
        ...
        if (latest && isPlainObject(latest)) {
          ...
          if (latest.attempt !== currentAttempt) {
            errors.push(
 ...
 * @param {string} [options.activeDir] - Override for `specs/active/` (test-only
 *   directory-injection seam — every real caller, `node tools/specs.mjs validate`
 *   included, omits this and gets the real repository directories).
 * @param {string} [options.archiveDir] - Override for `specs/archive/`, same as above.
 */
export function validateSpecs({ activeDir = ACTIVE_DIR, archiveDir = ARCHIVE_DIR } = {}) {
  const errors = [];
